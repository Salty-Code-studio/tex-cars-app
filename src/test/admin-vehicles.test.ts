...
describe("admin vehicles", () => {
  it("creates a vehicle with parsed defaults", async () => {
    const input = VehicleCreateSchema.parse({ ...base, slug: "test-wagon" });
  ...
  it("rejects a duplicate slug with a conflict", async () => {
    const input = VehicleCreateSchema.parse({ ...base, slug: "dupe-slug" });
    await createVehicle(input);
    await expectReject(createVehicle(input), /already exists/i);
  });

  it("validates the slug shape and class enum", () => {
    expect(VehicleCreateSchema.safeParse({ ...base, slug: "Not Kebab" }).success).toBe(false);
    expect(VehicleCreateSchema.safeParse({ ...base, slug: "ok", class: "Limo" }).success).toBe(false);
  });

  it("patches rates and deposit", async () => {
    const v = await createVehicle(VehicleCreateSchema.parse({ ...base, slug: "patch-me" }));
  ...
  it("retires instead of deleting, preserving the row", async () => {
    const v = await createVehicle(VehicleCreateSchema.parse({ ...base, slug: "retire-me" }));
  ...
  it("manages availability blocks per vehicle", async () => {
    const v = await createVehicle(VehicleCreateSchema.parse({ ...base, slug: "blocked-car" }));
