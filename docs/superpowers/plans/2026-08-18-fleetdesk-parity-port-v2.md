...
- MIGRATION RULE (supersedes wave-09 playbook item 5; AMENDED after wave 01): TEX journal idx 0 to 14 is fixed; idx 14 = `0014_majestic_sunspot` (admin_reset_tokens). FD `0014_hot_mesmero` (leads) is NEVER ported. FD `0015` to `0024` keep their exact file names and numbers in TEX; append `drizzle/meta/_journal.json` entries idx 15 onward with the FD tags in FD order. Do NOT preserve FD `when` values (drizzle's migrator uses `when` as its apply high-water mark; FD's values are lower than Tex's idx 14 and cause a silent skip on populated DBs): remap every appended entry with the binding formula in docs/PORT-LOG.md Note 9(b), `new_when = 1786970199527 + (fd_when - 1785176040444)`, keeping the sequence strictly increasing. Reconcile the matching `drizzle/meta/*_snapshot.json` (swap FD-only tables for Tex's admin_reset_tokens per Notes 8/9) and run the migration-smoke after every wave: `DATABASE_URL=pglite://.migration-smoke npm run db:migrate && rm -rf .migration-smoke` → exits clean. When FD later hardened a migration it shipped (as with 69b3889 for 0015), hoist the hardening commit alongside the migration itself.
